...
<body>
  ...
  <div class="full height">
    <div class="ui main container">
      <h1 class="ui center aligned header">編集画面</h1>
      <form class="ui form" action="/report/update" method="post">
        <div class="field">
          <label>タイトル</label>
          <input id="title" name="title" type="text" value="${reportForm.title}"/>
          <p class="error-messages"><c:out value="${errors.title}" /></p>
        </div>

        <div class="field">
          <label>内容</label>
          <textarea id="reportBody" name="reportBody">${reportForm.reportBody}</textarea>
        ...
        <div class="field">
          ...
          <input class="ui fluid green button" type="submit" value="編集する" name = "confirm" />
        </div>
        <div class="field">
          <a href="/report/detail?reportId=${reportId}" class="ui fluid button">戻る</a>
        </div>
      </form>
    </div>
  </div>
